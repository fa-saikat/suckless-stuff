...
/* 			/1* nmaster clients are stacked vertically, in the center of the screen *1/ */
/* 			resize(c, mx, my, mw - (2*c->bw), (mh / mfacts) + (i < mrest ? 1 : 0) - (2*c->bw), 0); */
/* 			my += HEIGHT(c) + ih; */
/* 		} else { */
...
/* 	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) */
/* 		if (i < m->nmaster) { */
/* 			/1* nmaster clients are stacked horizontally, in the center of the screen *1/ */
/* 			resize(c, mx, my, (mw / mfacts) + (i < mrest ? 1 : 0) - (2*c->bw), mh - (2*c->bw), 0); */
/* 			mx += WIDTH(c) + iv*mivf; */
/* 		} else { */
...
/* 		if (cols*cols >= n) */
/* 			break; */
/* 	if (n == 5) /1* set layout against the general calculation: not 1:2:2, but 2:3 *1/ */
/* 		cols = 2; */
/* 	rows = n/cols; */
